...
//			'swimming_pool'=>'',
		...
		if($request->input('room_type_id') && $request->input('type') && $request->input('page')) {
			return redirect ('/room/'.$request->input('type').'?page='.$request->input('page'));			
		}
		...
		$rooms->leftJoin('style_room', 'style_room.room_id',"=", 'rooms.id');
		
		if ($request->has('room_style_id')) {
			$rooms->where('style_id', $request->input('room_style_id'));
			
			$style['id'] = $request->input('room_style_id');
			  
		}
		 
		$rooms = $rooms->orderBy('id','DESC')->paginate(7);
		  
		if($request->input('room_type_id')) {
		$type = RoomTypeTranslation::where('room_type_id', $request->input('room_type_id'))->where('locale', 'ru')->first();
		}else{
		$type = RoomTypeTranslation::where('slug', $type)->where('locale', 'ru')->first()->roomType;
		}
...
elseif( $width > $height )
 ...
 $room['height2'] = $new_h;
	...
	$width2 = $picture->getWidth(0, ''); 
	$height2 = $picture->getHeight(0, '');
	
	$new_w2 = 300;
	
	$room['mb-width2'] = $new_w2;
