...
use App\Models\PcategoriesProducts;
...
class ProductsController extends Controller
{
    ...
    public function index(Request $request)
    {
		...
		if($name == '' && count($filters) == 0){
			$products = $products->latest()->paginate(20)->appends($request->all());
		} else {
            if ($name != '') {
                $products->whereHas('translations', function ($query) use ($name) {
                             $query->whereRaw('UPPER(name) LIKE "%'.$name.'%"');
                        });
            }
            if (count($filters) > 0) {
                $products->whereIn('pcategory_id',$filters);
                // dd($products);
            }
            $products = $products->latest()->paginate(20)->appends($request->all());
    ...
    public function create()
    {
        ...
        return view('admin.products.create',compact('manufacturers','rooms','tags','stores','pcategories', 'relatives', 'cities', 'storesWithCity'));
    }
...
//        return $image->image;
    ...
    public function store(CreateProductRequest $request)
    {
        ...
        localizationSlug($data,'name');
        $data['ru']['slug'] = $data['ru']['slug'].'-ru-'.date('His');
        $data['en']['slug'] = $data['en']['slug'].'-en-'.date('His');
        $product = Product::create($data);
        $product->stores()->attach($data['stores']);
        $product->store_cities()->attach($data['store_city']);
        if ($request->hasFile('image')) {
            ...
            foreach($files as $file){
                ...
                ProductPicture::create(['image' => $picture, 'product_id' => $product->id]); 
            }
        }
    ...
    public function edit($id)
    {
		...
		$categories = PcategoriesProducts::all(); 
        ...
        $cities = City::lists('name','id');
        $storesWithCity = StoreCity::with('city', 'store')->orderBy('store_id', 'ASC')->get();
        return view('admin.products.edit', compact('product','cities','tags','groupt','rooms','manufacturers','stores','pcategories','categories','relatives','product_stores', 'store_city', 'storesWithCity'));
    }
    ...
    public function update(UpdateProductRequest $request, $id)
    {
        ...
        try{
            ...
            if ($request->has('cities') && count($request->cities) > 0){
                ...
                foreach ($stores as $store) {
                    $cities = array_merge($cities, $store->cities()->lists('city_id'));                    
                }
                ...
                $product->store_cities()->sync($data['store_city']);

                //$product->category_product()->sync($data['test']); 
                if(isset($data['test2']))$product->room_product()->sync($data['test2']); 
                if(isset($data['tags']))$product->tag_product()->sync($data['tags']); 
				
                // $product->cities()->sync($cities);
            }
