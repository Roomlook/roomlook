    ...
    <div>
        ...
        <div class="col-md-12 room-slider-holder ">
            <div class="room-slider">
                <div class="hidden-md hidden-xs room-products">
                 ...
                 <div class="item pg-items" data-picture-id="{{ $picture->id }}" data-project-id="{{ $picture->room->project->id }}" data-project-pictures="{{ $picture->room->project->pictures(0, ['id', 'image']) }}">
                    ...
                    <figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
                      <a href="https://roomlook.com/{{ $picture->imagePath('md/') }}"   @if($picture->isSaved()) data-status="saved" @else data-status="" @endif data-picture-id="{{ $picture->id }}" itemprop="contentUrl" data-size="{{$picture->getWidth(0, '')}}x{{ $picture->getHeight(0, '')}}">
                          <img  <?php /*style="height:<?php echo ($room['height2'])?$room['height2'].'px':'auto'; ?>"*/ ?>
                                alt=""
                ...
                <div class="action-btns hidden-xs" style="width:40px;height:40px;">
                    
                        @if ($picture != NULL)
                                <a style="width:40px;height:40px;" href="#" tabIndex="-1" class="c_btn_transparent_green c_btn_small save-btn @if($picture->isSaved()) saved @endif" data-save-text="<i class='glyphicon glyphicon-ok'></i>" data-unsave-text="<img src='/images/download-arrow.png' />" data-model-id="{{ $picture->id }}" data-model-name="RoomPicture">
