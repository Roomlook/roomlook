                    ...
                    @if (isset($fromProject) && $fromProject)
						...
						*/ ?>
						...
						itemscope itemtype="http://schema.org/ImageObject">				
							...
							<?php /* @if($pic->isSaved()) data-status="saved" @else data-status="" @endif */ ?>
						
                            <a href="/images/rooms/original/{{ $pic->image }}" data-project-id="{{ $pic->room->project->id }}" 
							data-picture-id="{{ $pic->id }}"  itemprop="contentUrl" data-size="{{ $pic->getWidth(0, '') }}x{{ $pic->getHeight(0, '') }}">
                            ...
                            @foreach($pic->tags as $tag)
                                   <a href="/{{ $tag->product->image }}" tabIndex="-1" data-product-id="{{ $tag->product->id }}"  class="popup-product-open c_clear_btn" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">
                ...
                @foreach($roomsAll as $k => $r)
                    @if ($r->id == $room->id)
                         ...
                         <figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
						  ...
						  <?php /*@if($picture->isSaved()) data-status="saved" @else data-status="" @endif */ ?>
						  
						  <a href="/images/rooms/original/{{ $picture->image }}"   
						  data-picture-id="{{ $picture->id }}" data-project-id="{{ $picture->room->project->id }}" itemprop="contentUrl" data-size="{{ $picture->getWidth(0, '') }}x{{ $picture->getHeight(0, '') }}">
                            ...
                            <?php $i = 0; ?>
                            @foreach($picture->tags as $tag)
                           <a href="/{{ $tag->product->image }}" tabIndex="-1" data-product-id="{{ $tag->product->id }}"  class="    c_clear_btn" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">
                                <div class="hint tag" data-picture-id="{{ $picture->id }}"  data-room-id="{{ $room->id }}"  data-tag-id="{{ $tag->id }}" data-product-id="{{ $tag->product_id }}" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">  <div class="ring-container">
                                    <div class="ringring"></div>
                                    <div class="ringring2"></div>
                                    <div class="circle"></div>
...
@endif
                    ...
                    <div class="col-md-3 col-xs-3 ">
                        ...
                        <div class="desinger-image pull-left">
                            <img src="/{{ $room->project->author->image }}" class="designer img-responsive" alt="">
                        </div>
                        <div class="col-md-8">
                            <div class="pull-left">
