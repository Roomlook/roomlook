                    ...
                    @if (isset($fromProject) && $fromProject)
						...
						itemscope itemtype="http://schema.org/ImageObject">				
                            ...
                            @foreach($pic->tags as $tag)
                                ...
                                <div class="hint tag2" data-tag-id="{{ $tag->id }}"    data-room-id="{{ $room->id }}"  data-product-id="{{ $tag->product_id }}" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;"> 
                         ...
                         <figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
                          <a href="/images/rooms/original/{{ $picture->image }}" 
							...
							@if($picture->isSaved()) data-status="saved" @else data-status="" @endif data-picture-id="{{ $picture->id }}" data-project-id="{{ $picture->room->project->id }}" itemprop="contentUrl" data-size="{{ $picture->getWidth(0, '') }}x{{ $picture->getHeight(0, '') }}">
                              <img style="min-height:<?php echo ($room['height2'])?$room['height2'].'px':'auto'; ?>" 
                                    alt=""
                                        class="lazyload"  
                           ...
                           <a href="/images/rooms/original/{{ $tag->product->image }}" tabIndex="-1" data-product-id="{{ $tag->product->id }}"  class="    c_clear_btn" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">
                                <div class="hint tag2" data-picture-id="{{ $picture->id }}"  data-room-id="{{ $room->id }}"  data-tag-id="{{ $tag->id }}" data-product-id="{{ $tag->product_id }}" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">  <div class="ring-container">
                                    <div class="ringring"></div>
                                    <div class="ringring2"></div>
                                    <div class="circle"></div>
                                </div>
                    ...
                    @foreach($r->pictures as $pic)
                        @if ($pic->id != $picture->id)
                            <figure itemprop="associatedMedia" class="hidden-elements" itemscope itemtype="http://schema.org/ImageObject">
                                ...
                                @foreach($pic->tags as $tag)
                                    <a href="/images/rooms/original/{{ $tag->product->image }}" tabIndex="-1" data-product-id="{{ $tag->product->id }}"  class="popup-product-open    c_clear_btn" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">
                                    <div class="hint tag2" data-tag-id="{{ $tag->id }}"    data-room-id="{{ $r->id }}"  data-product-id="{{ $tag->product_id }}" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;"> 
                                        <div class="ring-container">
                                            <div class="ringring"></div>
                                            <div class="ringring2"></div>
                                            <div class="circle"></div>
