            ...
            <div class="room-slider">
                ...
                @foreach($roomsAll as $k => $r)
                    @if ($r->id == $room->id)
                         ...
                         <figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
                          <a href="https://roomlook.com/{{ $picture->imagePath('') }}"   @if($picture->isSaved()) data-status="saved" @else data-status="" @endif data-picture-id="{{ $picture->id }}" data-project-id="{{ $picture->room->project->id }}" itemprop="contentUrl" data-size="{{ $pic->getWidth(0, '') }}x{{ $pic->getHeight(0, '') }}">
                              <img  
                                    alt=""
                                        src="/{{ $picture->imagePath() }}"
                                        data-src="/{{ $picture->imagePath('') }}"  class="lazyload" tabIndex="-1" data-project-id="{{ $picture->room->project->id }}" data-picture-id={{ $picture->id }} 
