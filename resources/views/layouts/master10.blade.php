...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
	...
	#live-search.open {display:block;}
	#live-search img {width:100%;}
	#live-search ul li {padding:10px 0;}
	#live-search .product-description {padding-bottom:5px;color:#ccc;font-size:12px;}
	#live-search .thumb {
	width: 100%;
    height: 100px;
    display: block;
...
$.ajax({
  url: "/api/catalog/search?search=" + filter_name,
  cache: false,
  dataType: "json",
  success: function(res) {   
		...
		$('.result-text').html(''); 
				...
				$('.breadcumbs').html(res.bread);
				$('.gridview').html(res.data);
		...
		$.each(data, function(key, val){ 
													...
													html += '<li>';
													html += '<div class="col-md-4"><div><div class="thumb" style="background-image:url(' + img + ');"></div></div></div>';
													html += '<div class="col-md-8"><a href="/product/' + val.id + '">';
											 
													html += '<div class="product-name">' + val.name ; 
													html += '</div>';
													html += '<div class="product-description">' + val.name ; 
													html += '</div>';
											 
													html += '<span style="clear:both"><a class="btn-border-black">уточните цену</a></span>';
													html += '</a></div>';
...
<?php if(isset($_GET['catalog_tree'])) { ?>
$(document).ready(function () {
	
	$('.ajax-catalog #category_id').val(<?php echo $_GET['catalog_tree']; ?>);
		...
		var data = $(".ajax-catalog").serialize();
            ...
            $.ajax({
				...
				$('.breadcumbs').html(obj.bread);
				$('.cart i').html(obj.count);
				$('.gridview').html(obj.data);
...
$(document).on('click', '.reset', function(e) {  
	...
	$('.ajax-catalog .fcheckbox, .ajax-catalog input[name="group_id[]"]').remove();
	
	$('.ajax-catalog #category_id').val(0);
	
	$('.filter-tags label').click();
    ...
    $( "#amount" ).val( "" + $( "#slider-range" ).slider( "values", 0 ) +
      " - " + $( "#slider-range" ).slider( "values", 1 ) );
		...
		var data = $(".ajax-catalog").serialize();
            ...
            $.ajax({
                url: 'https://roomlook.com/ru/api/catalog2',
				data: data,
                type: 'post',
                success: function(res) {  
...
$(document).on('change', '.fcheckbox', function() { 
	...
	if(this.checked) {
		...
		$(this).clone().removeAttr('id').appendTo(".ajax-catalog");  
		 
		$('.filter-tags label[for="' + label_for + '"]').remove(); 
		$('.filter #' + label + ' + label').clone().appendTo(".filter-tags");
		
		var data = $(".ajax-catalog").serialize(); console.log(data);  
            ...
            $.ajax({
				...
				var obj = JSON.parse(res); console.log(obj.count); 
		...
		$('.filter-tags label[for="' + id + '"]').remove(); 
		$('.ajax-catalog input[data-val="' + id + '"]').remove(); 
		
		//
		$('.ajax-catalog input[value="' + group + '"]').remove(); 
		$("<input type='hidden' name='group_id[]' value='" + group + "' />").appendTo(".ajax-catalog");
...
<style>	 
header #main-menu .dropdown .menu-dropdown > div > ul > li > a {font-weight:600!important;}
header #main-menu .dropdown .menu-dropdown ul li ul li a {
    padding-left: 5px;
    position: relative;
    display: block;
    padding-top: 5px;
...
header #main-menu .dropdown .menu-dropdown ul li ul li a:before {display:none;}
header #main-menu .dropdown .menu-dropdown ul li {line-height:20px;margin:2px 0;}
...
-->
                        ...
                        <div class="col-md-3 col-xs-4 xs-nopadding">
                            ...
                            <button class="dropdown-toggle c_clear_btn" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
                                ...
                                <ul class="dropdown-menu" style="right:-50px;left:auto;" aria-labelledby="dropdownMenu1">
                            ...
                            <div class="footer clearfix">
                                ...
                                <div class="span3 col-md-3">
                                    <div class="widget">
                                        ...
                                        -->
                                            <li><a href="/p/o-proekte">Что такое RoomLook?</a></li>
                                            <li><a href="/p/kak-razmestit-proekt-na-sayte">Как разместить проект на сайте</a></li>
                                            <li><a href="#">Вопросы ответы</a></li>
                                            <li><a href="/p/pravila-bezopasnosti">Правила безопасности</a></li> 
										</ul>
                                    </div>
                                </div>
