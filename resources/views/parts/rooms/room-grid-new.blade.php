                          ...
                          <a href="/images/rooms/original/{{ $picture->image }}"   @if($picture->isSaved()) data-status="saved" @else data-status="" @endif data-picture-id="{{ $picture->id }}" data-project-id="{{ $picture->room->project->id }}" itemprop="contentUrl" data-size="{{$picture->getWidth(0, '')}}x{{ $picture->getHeight(0, '')}}">
                           ...
                           <a href="/images/rooms/original/{{ $tag->product->image }}" tabIndex="-1" data-product-id="{{ $tag->product->id }}"  class="    c_clear_btn" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">
                                <!--<div class="hint tag2" data-tag-id="{{ $tag->id }}"    data-room-id="{{ $r->id }}"  data-product-id="{{ $tag->product_id }}" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">-->
                                    <div class="hint tag2" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;" data-tag-id="{{ $tag->id }}" data-room-id="{{ $r->id }}" data-product-id="{{ $tag->product_id }}">   
									   <div class="ring-container ajax-ring-container">
                    ...
                    @if ($r->id == $room->id)
                         <figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
                          <a href="/images/rooms/original/{{ $picture->image }}"   @if($picture->isSaved()) data-status="saved" @else data-status="" @endif data-picture-id="{{ $picture->id }}" data-project-id="{{ $picture->room->project->id }}" itemprop="contentUrl" data-size="{{$picture->getWidth(0, '')}}x{{ $picture->getHeight(0, '')}}">
                            ...
                            @foreach($picture->tags as $tag)
                           <a href="/images/rooms/original/{{ $tag->product->image }}" tabIndex="-1" data-product-id="{{ $tag->product->id }}"  class="    c_clear_btn" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">
                               <!-- <div class="hint tag2" data-picture-id="{{ $picture->id }}"  data-room-id="{{ $room->id }}"  data-tag-id="{{ $tag->id }}" data-product-id="{{ $tag->product_id }}" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">  
								-->
		                         <div class="hint tag2" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;" data-tag-id="{{ $tag->id }}" data-room-id="{{ $r->id }}" data-product-id="{{ $tag->product_id }}">   
									   <div class="ring-container ajax-ring-container">
											<div class="ringring"></div>
...
@endif
                    ...
                    <div class="col-md-3 col-xs-3 ">
                        ...
                        <div class="desinger-image pull-left">
                            <img class="lazyload" data-src="/images/rooms/original/{{ $room->project->author->image }}" class="designer img-responsive" alt="">
                        </div>
                        <div class="col-md-8">
                            <div class="pull-left">
