...
<div class="room" 
	...
	author="{{ $room->project->author->user->name }}">
    <div>
        ...
        <div class="room-slider-holder ">
			...
			<div class="ajax-tag2" data-room-id="{{ $room->id }}">
            <div class="container">
                    <div class="row">
	...
	*/
    ...
    display: table-cell;
    vertical-align: middle;">
							<h3 class="tag-title">Идеально подходит</h3>
							 
							 
							<div class="ajax-gallery" data-room-id="{{ $room->id }}">
							</div>
							</div>
							</div>
							...
							<div class="ajax-tag-first">
							</div>
							 
								<div class="items ajax-tag-near" style="
	margin-top: -15px; 
    height: 260px;
    overflow: hidden;">  
							...
							<div class="custom-multi-items-carousel-controls vertical" id="custom-multi-items-carousel-controls-2_2" style="padding:20px;">
								<a href="javascript:void(0);" class="prev" style="display:none;">
									<img src="/images/str-00.png" />
								</a>
				...
				<?php 
						...
						$picture2 = $room->pictures()->first();
						//print_r($picture2->tags); 
                         ...
                         <div class="vertical-text" style="width: 100%">
                             <div> 
                                 {{ trans('frontend.common.designer') }} <a
                                     ...
                                     @if ($picture->room->project->photograph->user->name != 'нет' && $picture->room->project->photograph->user->name != '001')/ {{ trans('frontend.common.photo') }} <strong>{{ $picture->room->project->photograph->user->name }}</strong>@endif @endif
                             </div>
                         </div>
                     </div>
					 
                 <div class="item pg-items" 
					id="picture-{{ $picture->id }}"
					data-rid="{{ $room->id }}"
					data-picture-id="{{ $picture->id }}" 
					data-project-id="{{ $picture->room->project->id }}" 
					data-project-pictures="{{ $picture->room->project->pictures(0, ['id', 'image']) }}" 
					data-project-room="{{ $picture->room->project->rooms->count() }}">
                    ...
                    @if (isset($fromProject) && $fromProject)
                        ...
                        <figure class="hidden-elements test-{{ $pic->id }}-{{ $picture->id }}" 
						itemprop="associatedMedia"  
						itemscope itemtype="http://schema.org/ImageObject">				
						
                            <a href="/images/rooms/original/{{ $pic->image }}" 
							data-rid="{{ $room->id }}"
							data-project-id="{{ $pic->room->project->id }}" @if($pic->isSaved()) data-status="saved" @else data-status="" @endif data-picture-id="{{ $pic->id }}"  itemprop="contentUrl" data-size="{{ $pic->getWidth(0, '') }}x{{ $pic->getHeight(0, '') }}">
                                <img class="lazyload" data-src="/images/rooms/md/{{ $pic->image }}" itemprop="thumbnail" alt="Image description" data-project-id="{{ ($picture->room->project_id) }}">
                                 </a>
                            ...
                            @foreach($pic->tags as $tag)
                                ...
                                <div class="hint mb-tag" data-tag-id="{{ $tag->id }}"    data-room-id="{{ $room->id }}"  data-product-id="{{ $tag->product_id }}" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;"> 
                ...
                @foreach($roomsAll as $k => $r)
                    @if ($r->id == $room->id)
                         ...
                         <figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
                          <a href="/images/rooms/original/{{ $picture->image }}" 
							data-rid="{{ $room->id }}"
							@if($picture->isSaved()) data-status="saved" @else data-status="" @endif data-picture-id="{{ $picture->id }}" data-project-id="{{ $picture->room->project->id }}" itemprop="contentUrl" data-size="{{ $picture->getWidth(0, '') }}x{{ $picture->getHeight(0, '') }}">
                              <img data-width="<?php echo ($room['mb-width2'])?$room['mb-width2'].'px':'auto'; ?>"
									alt=""
                                        class="lazyload"  
                                        data-src="/images/rooms/original/{{ $picture->image }}"  
										tabIndex="-1" 
										data-project-id="{{ $picture->room->project->id }}" 
										data-picture-id={{ $picture->id }} 
                           ...
                           <a href="/images/rooms/original/{{ $tag->product->image }}" tabIndex="-1" data-product-id="{{ $tag->product->id }}"  class="    c_clear_btn" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">
                                <div class="hint mb-tag" data-picture-id="{{ $picture->id }}"  data-room-id="{{ $room->id }}"  data-tag-id="{{ $tag->id }}" data-product-id="{{ $tag->product_id }}" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">  <div class="ring-container">
                                    <div class="ringring"></div>
                                    <div class="ringring2"></div>
                                    <div class="circle"></div>
                                </div>
                    ...
                    @foreach($r->pictures as $pic)
                        @if ($pic->id != $picture->id)
                            <figure itemprop="associatedMedia" class="hidden-elements" itemscope itemtype="http://schema.org/ImageObject">
                                <a href="/images/rooms/original/{{ $pic->image }}" 
								data-rid="{{ $room->id }}"
								@if($pic->isSaved()) data-status="saved" @else data-status="" @endif data-picture-id="{{ $pic->id }}"  itemprop="contentUrl" data-size="{{ $pic->getWidth(0, '') }}x{{ $pic->getHeight(0, '') }}">
                                    <img class="lazyload" data-src="/images/rooms/md/{{ $pic->image }}" itemprop="thumbnail" alt="Image description" data-project-id="{{ ($picture->room->project_id) }}">
                                </a>
                                ...
                                @foreach($pic->tags as $tag)
                                    <a href="/images/rooms/original/{{ $tag->product->image }}" tabIndex="-1" data-product-id="{{ $tag->product->id }}"  class="popup-product-open    c_clear_btn" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">
                                    <div class="hint mb-tag" data-tag-id="{{ $tag->id }}" data-room-id="{{ $r->id }}"  data-product-id="{{ $tag->product_id }}" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;"> 
                                        <div class="ring-container">
                                            <div class="ringring"></div>
                                            <div class="ringring2"></div>
                                            <div class="circle"></div>
