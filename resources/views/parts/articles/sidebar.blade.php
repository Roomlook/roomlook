<aside>
	<div class="sidebar"><h2>Категории</h2> 
		<ul>
			...
			@if($category2->slug == $c->slug) 
			<li class="active"><a href="/ru/articles/{{ $c->slug }}">{{ $c->name }}</a></li> 
			@else
    ...
    font-size: 10px;">Показать все теги</a> -->
		...
		<?php /*
		<div class="artcles-tags" style="display:nnoe;">  
			@foreach($parts as $p) 
				<div style="opacity:1;"><a href="/ru/articles/tags/{{ $p['slug'] }}">{{ $p['title'] }}</a></div>
			@endforeach			
		</div>
		*/ ?>
	</div>
