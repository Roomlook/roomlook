...
<div class="form-horizontal">
    ...
    @endif 
		...
		<div class="form-group"> 
			<label for="square2" class="col-md-2 control-label">Првеью для главной:</label> 
			<div class="col-sm-9">
				{!! Form::file('image', ['class' => 'form-control', 'onchange' => 'previewFile2(this)', 'name' => 'cropimage2']) !!}
   
				<?php if($model->images2) { ?>
		<img src="/images/papers/<?php echo $model->images2; ?>" alt="" id="del-cropper2">
		...
		<div class="form-group"> 
			...
			<div class="col-sm-9">
				...
				<?php if($model->images) { ?>
		<img src="/images/papers/<?php echo $model->images; ?>" alt="" id="del-cropper">
	...
	function previewFile2(input) {
        ...
        var croppr2 = new Croppr('#cropper2', {
			startSize: [600, 600, 'px'],
            onInitialize: (instance) => { console.log(instance);   
			
			},
            onCropStart: (data) => { console.log('start', data);  

				$('input[name="width2"]').val(data.width);
				$('input[name="height2"]').val(data.height);
				$('input[name="x2"]').val(data.x);
				$('input[name="y2"]').val(data.y);
    ...
    <div class="form-group">
		...
		<select class="js-example-basic-multiple" name="categories[]" multiple="multiple">
			...
			<?php $part = explode(';', $model->categories); ?> 
			
			@foreach($categories as $c)	
			<?php if (in_array($c->paper_categories_id, $part)) { ?> 
			<option selected value="{{ $c->paper_categories_id }}">{{ $c->name }}</option> 
			<?php }else{ ?>
        ...
        function uploadFile(filesForm) {
            ...
            $.ajax({
                ...
                success: function (images) {  
					...
					node.className = 'owl-carousel once';					
					node.innerHTML = '<div class="item"><img src="https://roomlook.com/images/papers/articles/' + images['filename'] + '"></div><div class="item"><img src="https://roomlook.com/images/papers/articles/' + images['filename2'] + '"><img src="https://roomlook.com/images/papers/articles/' + images['filename3'] + '"></div>';
