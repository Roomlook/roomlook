...
<div class="room" data-room-id="{{ $room->id }}" author="{{ $room->project->author->user->name }}">
	...
	*/
    ...
    box-shadow: 0px 0px 15px #ababab;">
							...
							<h3 class="tag-title">Похожие товары</h3>
							<div class="ajax-tag-first"></div>
							<h3 class="tag-title">Похожие товары</h3>
							<div class="custom-multi-items-carousel vertical ajax-tag-near" 
								data-cus-carousel-dir="vertical" 
								data-cus-carousel-loop="true" 
								data-cus-carousel-show="3" 
								data-cus-carousel-speed="250" 
								data-cus-carousel-next="#custom-multi-items-carousel-controls-2_2 .next" 
								data-cus-carousel-prev="#custom-multi-items-carousel-controls-2_2 .prev"
							style="height:450px;padding:15px 0;" 
							>
                    ...
                    @if (isset($fromProject) && $fromProject)
                          ...
                          <a href="https://roomlook.com/{{ $picture->imagePath('') }}"   @if($picture->isSaved()) data-status="saved" @else data-status="" @endif data-picture-id="{{ $picture->id }}" data-project-id="{{ $picture->room->project->id }}" itemprop="contentUrl" data-size="{{$picture->getWidth(0, '')}}x{{ $picture->getHeight(0, '')}}">
                           ...
                           <a href="/{{ $tag->product->imagePath() }}" tabIndex="-1" data-product-id="{{ $tag->product->id }}"  class="    c_clear_btn" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">
                                <div class="hint tag2" data-picture-id="{{ $picture->id }}"  data-room-id="{{ $room->id }}"  data-tag-id="{{ $tag->id }}" data-product-id="{{ $tag->product_id }}" style="opacity:1;top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">
                        ...
                        <figure itemprop="associatedMedia" class="hidden-elements" itemscope itemtype="http://schema.org/ImageObject">
                            ...
                            @foreach($pic->tags as $tag)
                                ...
                                <div class="hint tag2" data-tag-id="{{ $tag->id }}"    data-room-id="{{ $room->id }}"  data-product-id="{{ $tag->product_id }}" style="opacity:1;top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;"> 
                    ...
                    @foreach($r->pictures as $pic)
                        @if ($pic->id != $picture->id)
                            <figure itemprop="associatedMedia" class="hidden-elements" itemscope itemtype="http://schema.org/ImageObject">
                                ...
                                @foreach($pic->tags as $tag)
                                    <a href="/{{ $tag->product->imagePath() }}" tabIndex="-1" data-product-id="{{ $tag->product->id }}"  class="popup-product-open    c_clear_btn" style="top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;">
                                    <div class="hint tag2" data-tag-id="{{ $tag->id }}"    data-room-id="{{ $r->id }}"  data-product-id="{{ $tag->product_id }}" style="opacity:1;top: {{ $tag->percent_top }}%; left: {{ $tag->percent_left }}%;"> 
