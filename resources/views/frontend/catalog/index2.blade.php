...
@section('title','RoomLook - Каталог')
@section('content')
<style> 
...
.sort .col-md-4:last-child .sitem {border-left:none;border-right:none;}
.sort .col-md-4:last-child .sitem:before {display:none;}
.sitem.on:before {
...
.filter-tags label {font-size:12px;margin:0px 5px 15px 5px;position:relative;padding-right:10px;cursor:pointer;}
...
<section id="main" class="catalog">
	...
	margin-top:15px;
	padding:0 10px;
    width: 100%;" type="text" placeholder="Поиск">
			</form>
...
<br>
					...
					<div class="col-md-2" style="height:30px;">
						...
						<div class="col-md-8">
						<form class="ajax-mb-catalog" style="opacity:0;">
...
*/ ?>
				<div class="filter-tags" style="height:20px;margin-top:-5px;">
...
@foreach($tags[1] as $t)
<li>
	<input type="checkbox" class="fcheckbox" value="{{ $t->tag_id }}" data-group="2" name="tag_id[]" id="style-{{ $t->tag_id }}" data-val="style-{{ $t->tag_id }}">
	<label for="style-{{ $t->tag_id }}" class="css-label">{{ $t->title }}</label>
</li>
@endforeach 
									</ul>
									</div>
...
@foreach($tags[0] as $t)
<li>
	<input type="checkbox" class="fcheckbox" value="{{ $t->tag_id }}" data-group="2" name="tag_id[]" id="color-{{ $t->tag_id }}" data-val="color-{{ $t->tag_id }}">
	<label for="color-{{ $t->tag_id }}" class="css-label">{{ $t->title }}</label>
</li>
@endforeach  
									</ul>
									</div>
...
@foreach($tags[2] as $t)
<li>
	<input type="checkbox" class="fcheckbox" value="{{ $t->tag_id }}" data-group="2" name="tag_id[]" id="material-{{ $t->tag_id }}" data-val="material-{{ $t->tag_id }}">
	<label for="material-{{ $t->tag_id }}" class="css-label">{{ $t->title }}</label>
</li>
@endforeach  
									</ul>
									</div>
...
@foreach($tags[4] as $t)
<li>
	<input type="checkbox" class="fcheckbox" value="{{ $t->tag_id }}" data-group="2" name="tag_id[]" id="forma-{{ $t->tag_id }}" data-val="forma-{{ $t->tag_id }}">
	<label for="forma-{{ $t->tag_id }}" class="css-label">{{ $t->title }}</label>
</li>
@endforeach  
									</ul>
									</div>
					...
					<div class="col-md-2" style="height:30px;display:none;">
...
<br />
    ...
    margin: 0 -10px;">
		...
		<ul>
            ...
            @endforeach 
                        <li>
                <a class="" data-room-id="0" href="/ru/catalog">Все разделы</a>
            </li>
		</ul>
